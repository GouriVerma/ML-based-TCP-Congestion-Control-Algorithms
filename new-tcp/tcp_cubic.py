...
class TcpCubic(TcpEventBased):
    ...
    def _cubic_window(self, t_us):
        """Calculate target window size using cubic function"""
        t = (t_us - self.epoch_start) / 1e6  # Convert to seconds
        target = self.C * (t - self.k)**3 + self.w_max
        
        # Apply TCP-friendly region adjustment
        if target < self.curr_cwnd:
            target = self.curr_cwnd
        return target
    ...
    def get_action(self, obs, reward, done, info):
        ...
        # Detect congestion event (ssThresh reduced)
        # if caState==3:
        #     # Update CUBIC parameters on congestion
        #     self.epoch_start = simTime_us
        #     self.w_max = cWnd
        #     self.k = ((self.w_max * (1 - self.beta)) / self.C) ** (1/3)
        #     # Multiplicative decrease on congestion
        #     new_ssThresh = int(max(2 * segmentSize, self.beta * cWnd))
       ...
       # IncreaseWindow
        ...
        # Calculate target window using cubic function
            # target_cWnd = self._cubic_window(simTime_us)
            
            # # Apply window growth
            # if cWnd < target_cWnd:
            #     new_cWnd = min(target_cWnd, cWnd + segmentSize)
            # else:
            #     # TCP-friendly region (linear growth)
            #     adder = segmentSize * segmentSize / cWnd
            #     new_cWnd = cWnd + int(max(1.0, adder))
