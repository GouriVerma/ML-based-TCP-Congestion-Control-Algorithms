...
TcpEventGymEnv::GetObservationSpace()
{
  ...
  //new to be copied from TimeBased
  // bytesInFlightSum
  // bytesInFlightAvg
  // segmentsAckedSum
  // segmentsAckedAvg
...
TcpEventGymEnv::GetObservation()
{
  ...
  /////////////////////////////New////////////////////////////////////////////////////////////
  //segmentsAckedSum
  // uint64_t segmentsAckedSum = std::accumulate(m_segmentsAckedVec.begin(), m_segmentsAckedVec.end(), 0);
...
TcpEventGymEnv::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  m_segmentsAckedVec.push_back(segmentsAcked);
...
TcpEventGymEnv::PktsAcked (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time& rtt)
{
  ...
  if (m_lastAckTime > MicroSeconds(0.0)) {
    Time ackInterval = now - m_lastAckTime;
    if (ackInterval.GetSeconds() > 0) { // Avoid division by zero
      double throughput = (segmentsAcked * m_tcb->m_segmentSize * 8) / ackInterval.GetSeconds();
